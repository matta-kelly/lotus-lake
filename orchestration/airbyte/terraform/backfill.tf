...
resource "null_resource" "shopify_backfill" {
  ...
  triggers = {
    # Stable trigger - only fires on resource creation/recreation
    # Workflow: set backfill: true → apply → set backfill: false → apply
    connection_id = airbyte_connection.shopify_to_lake.connection_id
  }

  provisioner "local-exec" {
    command = <<-EOT
      curl -s -X POST "${var.airbyte_server_url}v1/connections/reset" \
...
resource "null_resource" "klaviyo_backfill" {
  ...
  triggers = {
    # Stable trigger - only fires on resource creation/recreation
    connection_id = airbyte_connection.klaviyo_to_lake.connection_id
  }
