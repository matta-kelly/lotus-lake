/**
 * Processed Models Factory
 *
 * Auto-generates Cube.js cubes for all dbt models tagged 'processed'.
 * These are the int_* intermediate models (9 total).
 *
...
// Filter to processed models (tagged 'processed' OR name starts with 'int_')
const processedModels = Object.values(manifest.nodes)
  .filter(node => node.resource_type === 'model')
  .filter(node =>
    (node.tags && node.tags.includes('processed')) ||
    node.name.startsWith('int_')
  );

console.log(`[processed.js] Found ${processedModels.length} processed models`);

// Generate cube definitions
module.exports = processedModels.map(model => {
  const columns = parseColumnsFromSql(model.raw_code);
  const uniqueKey = model.config?.unique_key;

  console.log(`[processed.js] Generating cube: ${model.name} (${columns.length} columns: ${columns.slice(0, 5).join(', ')}${columns.length > 5 ? '...' : ''})`);
