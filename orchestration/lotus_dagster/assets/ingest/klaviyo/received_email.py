# assets/ingest/klaviyo/received_email.py
import logging
import time
...
# --------------------------------------------------------------------
# Flow-specific config for RECEIVED EMAIL EVENTS
# --------------------------------------------------------------------
NAMESPACE = "klaviyo"
...
METRIC_ID_RECEIVED_EMAIL = "Wr5CFC"
...
# --------------------------------------------------------------------
RECEIVED_EMAIL_SCHEMA = pa.schema([
...
# Asset builder
# --------------------------------------------------------------------
def build_received_email_assets():
    """Builds the asset pipeline for Klaviyo Received Email Events."""
    return make_flow_assets(NAMESPACE, TABLE, UPDATED_FIELD, extract_received_email_query_fn)
...
def extract_received_email_query_fn(client, last_sync: datetime) -> Dict:
    """
    Extracts 'Received Email' events from Klaviyo, passing raw data
    ...
    logger.info(f"Extracting Klaviyo RECEIVED EMAIL events from {start_iso} to {end_iso}")

    combined_filter = (
        f"and("
        f"equals(metric_id,'{METRIC_ID_RECEIVED_EMAIL}'),"
    ...
    try:
        ...
        logger.error(f"Failed to fetch Klaviyo received email events: {e}")
        raise RuntimeError(f"Klaviyo API error: {e}") from e

    elapsed = time.time() - start_time
    logger.info("EXTRACTION SUMMARY:")
    logger.info(f"  - Total received events: {len(all_events)}")
